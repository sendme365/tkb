...
**时间戳标注规则：**
- 生成费曼笔记前，扫描已读取的原始内容（raw 文件 / transcript / 粘贴文本），检测是否存在时间戳（常见格式：`HH:MM:SS`、`MM:SS`、`[00:01:23]`、`(1:23)` 等）
- 如果检测到时间戳：为「核心概念解析」每条概念和「逻辑架构梳理」每个论点，找到原文中对应片段最近的时间戳，标注在该条目**行尾**，格式：`` `⏱ MM:SS` ``（超过一小时用 `H:MM:SS`）
- 如果未检测到任何时间戳：忽略，不添加任何标记
- 时间戳仅作定位辅助，不强求每条都有（找不到对应片段时可省略）

### 6.9. 术语草稿提取

所有层生成完毕后，检查本次生成内容中保留了英文原样的关键技术术语。

1. 收集本次生成的中文内容（6b/6c/6d）中出现的、仍保持英文形式的技术词汇
2. 对照 terms.md（已在第六步开头读取），找出尚未收录的术语（忽略带 `# draft` 的行）
3. 对每个未收录术语，在 terms.md 末尾追加一行：
   ```
   <English term> = <中文译名或英文原词>  # draft
   ```
   若不确定应译还是保留，默认写 `= <English term>`，供人工审核
4. **只追加，不修改任何已有行**
