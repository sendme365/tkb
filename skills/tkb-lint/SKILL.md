...
1. 用 `bash "$SCRIPTS/find_wiki.sh" concepts`（`SCRIPTS="$HOME/.claude/plugins/marketplaces/tkb/scripts"`）列出所有 concept 文件（含子目录）
...
2. 用 `bash "$SCRIPTS/find_wiki.sh" concepts` 扫描所有 concept 文件（含子目录），提取 frontmatter `tags` 字段中的标签
3. 检测可能的不一致：
...
1. 用 `bash "$SCRIPTS/find_wiki.sh" concepts` 扫描所有 concept 文件（含子目录），检查是否有 `source_tag` frontmatter 字段
2. 用 `bash "$SCRIPTS/find_wiki.sh" analyses` 扫描所有 analysis 文件（含子目录），检查是否有 `source_tag` frontmatter 字段
...
1. 用 `bash "$SCRIPTS/find_wiki.sh" concepts` 和 `bash "$SCRIPTS/find_wiki.sh" analyses` 扫描所有文件（含子目录）
